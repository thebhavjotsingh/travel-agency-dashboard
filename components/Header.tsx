    ...
    return (
        <header className="header">
            <article>
                ...
                <p className={cn("text-grey-100 font-normal", location.pathname === '/' ? 'text-2xl md:text-lg' : 'text-sm md:text-lg')}>{description}</p>
            </article>
          {ctaText && ctaUrl && (
            <Link to={ctaUrl}>
              <ButtonComponent
                ...
                <span className="p-16-semibold text-white pr-1.5">{ctaText}</span>
              </ButtonComponent>
            </Link>
          )}
        </header>
